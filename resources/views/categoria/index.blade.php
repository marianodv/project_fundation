{{--
    ...
    @section('content')
        categoria.index template
    @endsection
--}}

<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight inline-block">Categorias</h2>
    </x-slot>
    <div class="md:col-span-2 ms:mt-0 w-4/5 mx-auto mt-8">
    <div class="flex flex-col bg-white px-4 py-5 sm:p-6">
        <div class="flex space-x-2 justify-end">
            <a href="{{route('categoria.create')}}"><button type="button" class="inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Agregar</button></a>
          </div>
                ...
                <tbody>
                    @foreach ($categorias as $categoria)
                      <tr class="border-b hover:bg-gray-100">
                        ...
                        <td><form action="{{route('categoria.destroy',$categoria->id)}}" method="POST">
                            @csrf
                            @method('delete')
